...
export async function GET(req) {
  ...
  match.$and = words.map(word => ({
    $or: [
      ...
      { group_title: { $regex: word, $options: 'i' } },
      { group_slug: { $regex: word.replaceAll(' ', '-'), $options: 'i' } },
      { category_slug: { $regex: word.replaceAll(' ', '-'), $options: 'i' } },
      { category_item: { $regex: word, $options: 'i' } },
