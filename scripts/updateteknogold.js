...
async function main() {
  try {
      ...
      for (let i = 1; i <= pages; i++) {
        ...
        const products = await page.$$eval('.ItemOrj', (items) => {
          return items.map((el) => {
            ...
            const imgEl = el.querySelector('.productImage img');
        ...
        for (const product of products) {
          try {
            ...
            // Ürün zaten varsa sadece fiyat ve görsel güncelle
            const existing = await Product.findOne({ productUrl });
            if (!existing) {
              console.log(`⏭️ Atlandı (yeni ürün eklenmeyecek): ${product.name}`);
              continue;
            }
    ...
    console.error('❌ Genel scraper hatası:', err);
    process.exit(1);
  }
}
